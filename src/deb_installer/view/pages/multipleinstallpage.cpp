...
MultipleInstallPage::MultipleInstallPage(DebListModel *model, QWidget *parent)
    ...
    , m_infoControlButtonFrame(new QWidget(this))
...
void MultipleInstallPage::initUI()
{
    ...
    initInstallInfoFrame();
    initInstallProcessFrame();
    initTipsFrame();
    initButtonsFrame();
}

void MultipleInstallPage::initDebListViewFrame()
{
    ...
    m_centralLayout->addWidget(m_appsListViewBgFrame, Qt::AlignCenter);
}

void MultipleInstallPage::initInfoControlButtonFrame()
{
    QVBoxLayout *controlBtnLayout = new QVBoxLayout();
    controlBtnLayout->setContentsMargins(0, 10, 0, 5);

    m_infoControlButton->setVisible(false);                                                     //详细按钮展开收缩默认不可见
    controlBtnLayout->addWidget(m_infoControlButton);

    m_infoControlButtonFrame->setLayout(controlBtnLayout);

    m_centralLayout->addWidget(m_infoControlButtonFrame, Qt::AlignCenter);
}

void MultipleInstallPage::initInstallInfoFrame()
{
    // 设置安装信息的信息展示框
    ...
    m_installProcessInfoView->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);

    QVBoxLayout *installInfoLayout = new QVBoxLayout();
    installInfoLayout->setContentsMargins(20, 0, 0, 0);

    installInfoLayout->addWidget(m_installProcessInfoView, Qt::AlignCenter);

    m_installInfoFrame->setLayout(installInfoLayout);
    m_centralLayout->addWidget(m_installInfoFrame, Qt::AlignCenter);
}

void MultipleInstallPage::initInstallProcessFrame()
{
    //进度布局
    QVBoxLayout *progressFrameLayout = new QVBoxLayout();
    progressFrameLayout->setSpacing(0);
    progressFrameLayout->setContentsMargins(0, 5, 0, 0);                                        //设置边距
    ...
    progressFrameLayout->setAlignment(m_installProgress, Qt::AlignCenter);                     //进度条居中
    progressFrameLayout->addStretch();
    ...
    m_centralLayout->addWidget(m_processFrame);
}

void MultipleInstallPage::initTipsFrame()
{
    //修复依赖安装提示语会有焦点的问题。
    ...
    tipsLayout->addWidget(m_dSpinner);
    tipsLayout->addSpacing(4);
    tipsLayout->addWidget(m_tipsLabel);

    m_tipsFrame->setLayout(tipsLayout);
    m_tipsFrame->setVisible(false);
    m_centralLayout->addWidget(m_tipsFrame, Qt::AlignCenter);
}

void MultipleInstallPage::initButtonsFrame()
{
    ...
    btnsLayout->setContentsMargins(0, 20, 0, 20);                                                //底部间距30


    m_buttonsFrame->setLayout(btnsLayout);

    m_centralLayout->addWidget(m_buttonsFrame);
}
